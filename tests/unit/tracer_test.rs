...
mod tests {
    ...
    fn test_tracer_creation() {
        let tracer = Tracer::new("Test Tracer", RunType::Chain, json!({"input": "test"}));
        
        assert_eq!(tracer.run.name, "Test Tracer");
    ...
    fn test_tracer_create_child() {
        let parent = Tracer::new("Parent", RunType::Chain, json!({}));
    ...
    fn test_tracer_with_context() {
        ...
        assert_eq!(tracer.run.thread_id, Some("thread-123".to_string()));
        assert_eq!(tracer.run.session_name, Some("test-project".to_string()));
    }
    ...
    fn test_tracer_context() {
        ...
        // Context should have trace_id (even if not set, uses run.id)
        assert!(context.trace_id == tracer.run.id || tracer.run.trace_id == Some(context.trace_id));
    }
    ...
    fn test_tracer_run_id() {
        ...
        assert_eq!(run_id, tracer.run.id);
    }
}
